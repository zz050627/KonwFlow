...
**文件位置**：`bootstrap/src/main/java/com/nageoffer/ai/ragent/RagentApplication.java`
...
```bash
# 方式1：直接运行jar包（推荐生产环境�?java -jar bootstrap/target/ragent-0.0.1-SNAPSHOT.jar

# 方式2：Maven插件运行（开发环境）
mvn spring-boot:run -pl bootstrap

# 方式3：IDE运行
# 直接运行 RagentApplication.main()

# 方式4：使用启动脚本（Windows�?start-ragent.cmd
```
...
2. `application-{profile}.yaml`：环境特定配置（�?`application-dev.yaml`�?3. `ragent-secrets.yaml`：敏感信息（通过 `spring.config.import: optional:file:./ragent-secrets.yaml` 导入�?4. 环境变量：如 `OLLAMA_BASE_URL`
...
spring:
  profiles:
    active: dev  # 激活的环境
  config:
    import: optional:file:./ragent-secrets.yaml  # 导入敏感配置
```
...
**核心 API 路由**�?```
POST   /api/ragent/rag/v3/chat              # SSE流式对话
GET    /api/ragent/rag/settings             # 获取RAG设置
POST   /api/ragent/knowledge/base           # 创建知识�?POST   /api/ragent/knowledge/document       # 上传文档
GET    /api/ragent/knowledge/chunk          # 查询分块
POST   /api/ragent/user/login               # 用户登录
GET    /api/ragent/rag/trace/{taskId}       # 查询链路追踪
```

**路由前缀**：`server.servlet.context-path: /api/ragent`
...
**成功启动的标�?*�?```
2026-04-08 12:00:00.123  INFO 12345 --- [main] c.n.a.r.RagentApplication : Started RagentApplication in 8.234 seconds
