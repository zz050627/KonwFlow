...
**使用位置**�?- `bootstrap/src/main/java/com/nageoffer/ai/ragent/**/dao/mapper/`：所�?MyBatis Mapper
...
**消费失败处理**�?- 自动重试：最�?�?- 死信队列：`%DLQ%ragent-consumer`
...
**文件路径规则**�?```
http://localhost:9000/ragent/documents/{date}/{uuid}.{ext}
示例：http://localhost:9000/ragent/documents/20260408/abc123.pdf
```
...
3. 检查消费者组：`mqadmin consumerProgress -n localhost:9876 -g ragent-consumer`
